  ...
  try {
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < posts.length; i++) {
        ...
        print(
            "*****${getIt<SharedPreferences>().setInt('post_id', id).toString()}");
...
Future<ResultModel> add_post_tofav_ser(int id) async {
  ...
  print(id);
  Response response = await dio.post(BaseUrl + "addToFavorite/${id.toString()}",
      options: Options(
        headers: getoptions(),
...
Future<ResultModel> my_post_ser() async {
  Dio dio = Dio();
  Response response = await dio.get(BaseUrl + "showMyPosts",
      options: Options(headers: getoptions()));
  try {
    if (response.statusCode == 200) {
      print(response.statusCode);
      List<PostModel> mypost_List = List.generate(response.data["data"].length,
          (index) => PostModel.fromMap(response.data["data"][index]));
